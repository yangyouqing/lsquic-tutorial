...
struct tut
{
    ...
    int                         tut_sock_fd;    /* socket */ // for  read
    ev_io                       tut_sock_w;     /* socket watcher */
    ev_timer                    tut_timer;
    ev_timer                    tut_timer_delaysend;
...
#define MAX_SEND_BUF 1024
    ssize_t nw;
    static FILE* fp = NULL;
    static int last_remained = 0;
    static uint32_t last_sent = 0;
    char buf[MAX_SEND_BUF];
    size_t nread = 0;

    if (time(NULL) - last_sent < 1) {
        LOG("wait for a while, cur:%d, last sent:%d\n", time(NULL), last_sent);
        return;
    }
    ...
    if (0 == last_remained) {  
        ...
        nw = lsquic_stream_write(stream, buf, nread);
    } else {
        nread = last_remained;
        nw = lsquic_stream_write(stream, buf+ sizeof(buf) - last_remained, last_remained);
    }
    

    last_sent = time(NULL);
    last_remained = nread - nw;
    if (nread != nw) {
        LOG("diff read: %d/write:%d\n", nread, nw);
    }
    
    if (nw > 0)
    {
        LOG("send byte: %d, time: %d\n", nw, time(NULL));
    } else if (0 == nw) {
        
            lsquic_stream_wantwrite(stream, 0);
       
        LOG("send byte 0, time: %d\n", time(NULL));
    }
    else if (nw < 0)
    {
...
static    file_ctx_t file_ctx = {.fp=NULL, .left_size=0};


size_t file_read (void *lsqr_ctx, void *buf, size_t count)
{
    file_ctx_t *pFile_ctx = lsqr_ctx;
    if (NULL == lsqr_ctx) {
        return 0;
    }
#if 1

    if (NULL == pFile_ctx->fp) {
        LOG("FILE null");
        return 0;
    }
    size_t ret = fread(buf, 1, count, pFile_ctx->fp);
    pFile_ctx->read_size = ret;
    pFile_ctx->left_size -= ret;
    return ret;
#else 
    strncpy (buf, "CMD_SEND_FILE_START\n", count);
    pFile_ctx->left_size = 0;
    return 12;
#endif
}

size_t file_size (void *lsqr_ctx)
{
    file_ctx_t *pFile_ctx = lsqr_ctx;
    if (NULL == pFile_ctx) {
        return 0;
    ...
    if (0 == pFile_ctx->left_size) {
        fclose(pFile_ctx->fp);
    }
    return pFile_ctx->left_size;
}
 ...
 */
static void
tut_client_on_write2 (struct lsquic_stream *stream, lsquic_stream_ctx_t *h)
{
    lsquic_conn_t *conn;
    struct tut *tut;
    ssize_t nw;
    ...
    if (NULL == file_ctx.fp) {
        ...
        file_ctx.fp = fopen(filename, "r");
        if (NULL == file_ctx.fp) {
            LOG("OPEN file %s filed, errno: %d\n", filename, errno);
            return;
        }

        fseek(file_ctx.fp, 0, SEEK_END);
        file_ctx.left_size = ftell(file_ctx.fp);
        fseek(file_ctx.fp, 0, SEEK_SET);
        LOG("FILE SIZE: %d\n", file_ctx.left_size);
    }

    
    reader.lsqr_ctx = &file_ctx;
    reader.lsqr_read = file_read;
    reader.lsqr_size = file_size;

    if (reader.lsqr_size(reader.lsqr_ctx) <= 0) {
        LOG("FILE send done\n");
        lsquic_stream_write(stream, &nw, 0);
...
main (int argc, char **argv)
{
    ...
    /* Check settings */
    if (0 != lsquic_engine_check_settings(&settings, 0,
    ...
    tut.tut_engine = lsquic_engine_new(0, &eapi);
    if (!tut.tut_engine)
    {
    ...
    tut.tut_timer_delaysend.data = &tut;
